...
MemoryMonitor::MemoryMonitor(MyAllocator::MyAllocator<1024 * 16> *allocator, QWidget *parent)
...
void color(MyAllocator::Additional::header const **h) {
	if ((*h)->state)
		glColor3f(1, 0.8, 0.8);
	else
		glColor3f(0.8, 1, 0.8);
...
void MemoryMonitor::paintGL() {
	...
	m_allocator->monitor([begin, this](MyAllocator::Additional::header const **h) {
		size_t start = h - begin;
		size_t end = (*h)->next - begin;
		size_t ex = end % cells;
		size_t ey = end / cells;
